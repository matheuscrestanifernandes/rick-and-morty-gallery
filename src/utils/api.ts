...
export async function fetchCharactersByStatus(status: string): Promise<Character> {
  const characters = await axios.get(`${BASE_URL}/character/?status=${status}`);
  return characters.data.results;
}

export async function fetchCharactersByGender(gender: string): Promise<Character> {
