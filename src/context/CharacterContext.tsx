...
export function CharacterProvider({ children }: CharacterContextProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const fetchedCharacters = await getCharacters(pageNumber as number);
        setCharacters(
  ...
  async function getIDs(): Promise<number[]> {
    try {
      const jsonIDs = await localStorage.getItem(storageKey);
      if (jsonIDs === null) {
        return [];
      }
  ...
  async function toggleFavorite(image: any): Promise<boolean> {
    ...
    if (JSON.stringify(existingIDs).includes(image.name)) {
      ...
      await localStorage.setItem(storageKey, JSON.stringify(existingIDs));
      return true;
    }

    const newIDs = [...existingIDs, image];
    await localStorage.setItem(storageKey, JSON.stringify(newIDs));
    return true;
  }
